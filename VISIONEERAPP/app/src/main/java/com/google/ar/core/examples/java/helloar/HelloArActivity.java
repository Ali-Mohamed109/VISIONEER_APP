...
import com.example.visioneerapp.R;
...
public class HelloArActivity extends AppCompatActivity implements SampleRender.Renderer {
  ...
  private float scaleFactor = 0.01f; // Scale factor for virtual objects
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    increaseButton.setOnClickListener(new View.OnClickListener() {
      @Override
      public void onClick(View v) {
        scaleFactor += 0.01f; // Increase scale factor
      }
    });
    ...
    decreaseButton.setOnClickListener(new View.OnClickListener() {
      @Override
      public void onClick(View v) {
        scaleFactor -= 0.01f; // Decrease scale factor
        if (scaleFactor < 0) scaleFactor = 0; // Ensure scale factor doesn't go below 0
      }
    });
    Button selectFileButton = findViewById(R.id.select_obj_file_button);
    ...
    if (requestCode == 1 && resultCode == Activity.RESULT_OK) {
      ...
      if (resultData != null) {
        try {
          ...
          Uri.parse(uri.toString()); // This line will throw an exception if the URI is not valid
          Log.i(TAG, "Uri: " + uri.toString());
          // Save the URI for later use
          Uri selectedObjUri = uri;
        } catch (IllegalArgumentException e) {
          Log.e(TAG, "Invalid URI", e);
          // Handle the error here. For example, you could show a Toast or a dialog to the user
        }
      }
    }
  }
  ...
  public void onSurfaceCreated(SampleRender render) {
    ...
    try {
      ...
      virtualObjectMesh = Mesh.createFromAsset(render, "models/Cottage_FREE.obj");
      virtualObjectShader =
          Shader.createFromAssets(
                  render,
...
//      float scaleFactor = 0.01f; // Adjust this value to scale your model
